...
<div class="container">
    ...
    <!--搜索-->
    <input type="hidden" name="id" id="id1" value="${param.id}"/>
    <input type="hidden" name="name" id="name1" value="${param.name}"/>
    <input type="hidden" name="catelogId" id="catelogId1" value="${param.catelogId}"/>
    ...
    <nav class="text-center">
        <ul class="pagination">
            ...
            <c:if test="${goodsGrid.current ne 1 }">
                <li><a class="a1"
                        href="<%=basePath%>admin/searchGoods?pageNum=${goodsGrid.current-1 }">上一页</a>
                </li>
            </c:if>

            <c:if test="${goodsGrid.current < (goodsGrid.total+9)/10-1 }">
                <li><a class="a1"
                        href="<%=basePath%>admin/searchGoods?pageNum=${goodsGrid.current+1 }">下一页</a>
                </li>
            </c:if>
        </ul>
    </nav>
...
<script type="text/javascript">
    ...
    $(function(){
        $(".a1").click(function(){
            var id = $("#id1").serialize();
            var name = $("#name1").serialize();
            var catelogId = $("#catelogId1").serialize();

            var href = this.href + "&" + id+"&"+name+"&"+catelogId;
