...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<User> getPageUserByUser1(String phone, String username, Integer status, int pageNum, int pageSize) {
        PageHelper.startPage(pageNum,pageSize);//分页核心代码
        List<User> list= userMapper.getUserListByUser1(phone,username,status);
        return list;
    }

    @Override
    public int getUserNumByCondition(String phone, String username, Integer status) {
        List<User> list= userMapper.getUserListByUser1(phone,username,status);
        return list.size();
    }
